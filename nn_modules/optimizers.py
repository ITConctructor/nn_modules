...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for i in range(len(parameters)):
            ...
            """
            gt = gradients[i]
            gt = gt + self.weight_decay*parameters[i]
            self.state["m"][i] = self.momentum*self.state["m"][i] + gt
            parameters[i] = parameters[i]-self.lr*self.state["m"][i]
            np.add(parameters[i], 0, out=self.module.parameters()[i])
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for i in range(len(parameters)):
            ...
            self.state['m'][i] = self.beta1*self.state['m'][i] + (1 - self.beta1)*gt
            self.state['v'][i] = self.beta2*self.state['v'][i] + (1 - self.beta2)*np.square(gt)
            m_roof = self.state['m'][i]/(1-self.beta1**self.state['t'])
            v_roof = self.state['v'][i]/(1-self.beta2**self.state['t'])
            param = param - self.lr*m_roof/(np.sqrt(v_roof)+self.eps)
            np.add(param, 0, out=self.module.parameters()[i])
